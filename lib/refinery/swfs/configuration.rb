module Refinery
  module Swfs
    ...
    self.dragonfly_url_format = '/system/swfs/:job/:basename.:format'
    self.trust_file_extensions = false
    self.max_file_size = 524288000
    self.pages_per_dialog = 7
    self.pages_per_admin_index = 20
    self.whitelisted_mime_types = %w(swf/swf)

    class << self
      def datastore_root_path
        config.datastore_root_path || (Rails.root.join('public', 'system', 'refinery', 'swfs').to_s if Rails.root)
      end
